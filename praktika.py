...
import math
df_gender = pd.read_csv("/home/abdukarimov/O'zbekistan aholi demografiyasi/byGender.csv")

df_gender= df_gender.transpose()
df_gender.drop(df_gender.index[0], inplace=True)
df_gender.rename(columns={0: "Ayollar", 1 : "Erkaklar",}, inplace=True)
df_gender.index = df_gender.index.astype('int')
...
def update_gender_graph(years):
    df_filtered_gender = df_gender[df_gender.index == years]
